...
describe("Testing DemoQA", () => {
    ...
    it.skip('Alert is thrown when user already exists', () => {
      ...
      cy.get('#sign-username').type('jefta', { delay: 0 });
      cy.get('#sign-password').type('jefta123', { delay: 0 });
    ...
    it.skip("Adding product to cart", () => {
      ...
      cy.get('#name').clear().type('Nikola Jevtic');
      cy.get('#country').clear().type('Srbija');
      cy.get('#city').clear().type('Leskovac');
      cy.get('#card').clear().type('2947386185');
      cy.get('#month').clear().type('4');
