...
describe("Testing DemoBlaze", () => {
    ...
    it.skip('Alert is thrown when user already exists', () => {
      ...
      cy.get('#sign-username').type('azdravkovic', { delay: 0 });
      cy.get('#sign-password').type('123321', { delay: 0 });
    ...
    it("Adding product to cart", () => {
      ...
      cy.get('#name').clear().type('Aleksandar Zdravkovic');
      cy.get('#country').clear().type('Srbija');
      cy.get('#city').clear().type('Smederevska Palanka');
      cy.get('#card').clear().type('32245876198');
      cy.get('#month').clear().type('07');
